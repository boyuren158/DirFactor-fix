...
DirFactor.fix = function( data, hyper, start = NA, save.path = NA, 
  ...
  #dimension of data
  n = ncol( data )
  n.sub = nrow( hyper$sub.design )
  sub.rep = rowSums( hyper$sub.design )
  p = nrow( data )
  ...
  if(is.na(start)){
    ...
    Y.sub.old = matrix( rnorm( hyper$m*n.sub ), nrow = hyper$m )
    ...
    phi.old = matrix( rgamma( hyper$m*n.sub, shape = hyper$nv/2, rate = hyper$nv/2 ), nrow = n.sub )
    x.old = matrix( rnorm( p*nrow(hyper$y.fix) ), nrow=nrow(hyper$y.fix) )
    d.old = rgamma( nrow(hyper$y.fix), 1, 1 )
  }else{
  ...
  Y.old = Y.sub.old%*%hyper$sub.design
  d.old.diag = diag( d.old, nrow=length(d.old), ncol=length(d.old))
  ...
  for( iter in 1:step ){
    ...
    Q.para.all = cbind( c(data), c(t(X.old)%*%Y.old + t(x.old)%*%d.old.diag%*%hyper$y.fix), 
    ...
    if( 'Y.sub' %in% save.obj ){
      all.cache$Y.sub = Y.sub.old
    }
    Y.mean.pre = X.old%*%(Q.old-t(x.old)%*%d.old.diag%*%hyper$y.fix)%*%t(hyper$sub.design)/er.old
    ...
    er.old = 1/rgamma( 1, shape = n*p/2+hyper$a.er, 
                       rate = sum((Q.old-t(X.old)%*%Y.old-t(x.old)%*%d.old.diag%*%hyper$y.fix)^2)/2 + 
                         hyper$b.er )
    ...
    X.mean = t(Sigma.X)%*%Y.old%*%t(Q.old-t(x.old)%*%d.old.diag%*%hyper$y.fix)/er.old
    ...
    phi.old = matrix( rgamma( n.sub*hyper$m, shape = (hyper$nv+1)/2, rate = c( Y.sub.old^2*tau.tmp + hyper$nv )/2 ), 
                      nrow = n.sub, byrow = T )
    ...
    for( k in 1:hyper$m ){
      if( k == 1 ){
        ...
        delta.tmp[k] = rgamma( 1, shape = n.sub*hyper$m/2 + hyper$a1, 
                               rate = 1 + sum(colSums( t(Y.sub.old^2)*phi.old )*delta.prime)/2
        )
      }
      else{
        delta.prime = cumprod( delta.tmp )[-(1:(k-1))]/delta.tmp[k]
        delta.tmp[k] = rgamma( 1, shape = n.sub*(hyper$m-k+1)/2 + hyper$a2, 
    ...
    #sample the fixed effect global effect d.old
    if( 'd' %in% save.obj ){
      all.cache$d = d.old 
    }
    d.cov = solve( (x.old%*%t(x.old)*hyper$y.fix%*%t(hyper$y.fix))/er.old + diag( rep( 1, nrow(hyper$y.fix) ) ) )
    d.mean = as.vector( d.cov%*%diag( x.old%*%(Q.old-t(X.old)%*%Y.old)%*%t(hyper$y.fix)/er.old ) )
    # rejection sampling inefficient when length(d) is large
    if( length(d.old)>10 ){
      d.old = as.vector( tmvtnorm::rtmvnorm( 1, mean = d.mean, sigma = d.cov, lower = rep( 0, nrow( d.cov ) ), 
                                   algorithm = "gibbs" ) )
    }
    else{
      d.old = as.vector( tmvtnorm::rtmvnorm( 1, mean = d.mean, sigma = d.cov, lower = rep( 0, nrow( d.cov ) ) ) )
    ...
    x.cov = solve( d.old.diag%*%(hyper$y.fix%*%t(hyper$y.fix))%*%d.old.diag/er.old + 
                     diag( rep( 1, nrow(hyper$y.fix) ) ) )
    x.mean.all = x.cov%*%d.old.diag%*%hyper$y.fix%*%t(Q.old-t(X.old)%*%Y.old)/er.old
    x.old = x.mean.all + t( mvtnorm::rmvnorm( ncol(x.mean.all), sigma = x.cov ) )
    
    if( iter > burnin*step & iter%%thin == 0 )
      saveRDS( all.cache, file = paste( save.path, iter, sep = "_") )
  }
  
  t.t.1 = proc.time()
  return( list(running.time=t.t.1-t.t, save.path = save.path, 
               sub.design = hyper$sub.design, y.fix = hyper$y.fix))
}